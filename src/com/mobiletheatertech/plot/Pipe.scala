...
class Pipe(element: Element, parent: MinderDom) extends UniqueId(element)
  ...
  def parentParse(): Unit = {
    if (classOf[PipeBase].isInstance(parent)) {
      ...
      println( "Found that parent is a pipe and got " + parentPipe.name)
      println("Mounting this pipe " + getStringAttribute("id")
        + " on " + parentPipe.getStringAttribute("id")
        + " attach " + getOptionalDoubleAttributeOrNull("attach"))
    } else {
      println( "No idea what is going on")
    }
    //else
    //      null
  }
